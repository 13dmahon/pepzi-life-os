...
export default function ScheduleSetup({ onComplete, onBack }: ScheduleSetupProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Navigate directly instead of calling onComplete
      router.push('/today');
    } catch (err) {
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4 py-12">
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-slate-800">Set up your schedule</h1>
            <p className="text-slate-500 mt-1">This helps us plan around your life</p>
          </div>
          ...
          <GlassCard className="p-6 min-h-[420px] flex flex-col">
            ...
            <div className="flex-1">
              {step === 0 && <SleepScheduleStep data={data} updateData={updateData} />}
              {step === 1 && <WorkScheduleStep data={data} updateData={updateData} />}
              {step === 2 && <CommuteStep data={data} updateData={updateData} />}
              {step === 3 && <CommitmentsStep data={data} updateData={updateData} />}
            </div>
            ...
            <div className="flex gap-3 mt-6 pt-4 border-t border-white/40">
              ...
              {step < totalSteps - 1 ? (
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex items-center gap-2 px-6 py-2.5 bg-slate-800 text-white rounded-2xl font-medium hover:bg-slate-700 transition-all shadow-lg disabled:opacity-50"
                >
                  {loading ? (
                    <>
...
function SleepScheduleStep({ data, updateData }: { data: ScheduleData; updateData: (updates: Partial<ScheduleData>) => void }) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-slate-800">Your daily rhythm</h2>
        <p className="text-slate-500 mt-1">When do you wake up and go to sleep?</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-white/50 rounded-2xl p-4 border border-white/60">
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700 text-lg"
          />
        </div>
...
function WorkScheduleStep({ data, updateData }: { data: ScheduleData; updateData: (updates: Partial<ScheduleData>) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-slate-500 mt-1">Do you have regular work hours?</p>
      </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-medium transition-all ${
            data.works ? 'bg-slate-800 text-white shadow-lg' : 'bg-white/50 text-slate-500 hover:bg-white/70 border border-white/60'
          }`}
        >
          Yes
        </button>
                ...
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
                    data.work_days.includes(day)
                      ...
                      : 'bg-white/50 text-slate-400 hover:bg-white/70 border border-white/60'
                  }`}
                >
                  {DAY_LABELS[i]}
                </button>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700"
              />
            </div>
            <div>
...
// Step 3: Commute
function CommuteStep({ data, updateData }: { data: ScheduleData; updateData: (updates: Partial<ScheduleData>) => void }) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-slate-800">Commute</h2>
        <p className="text-slate-500 mt-1">Do you commute to work?</p>
      </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-medium transition-all ${
            data.has_commute ? 'bg-slate-800 text-white shadow-lg' : 'bg-white/50 text-slate-500 hover:bg-white/70 border border-white/60'
          }`}
        >
          Yes
        </button>
      ...
      {data.has_commute && (
        <div className="space-y-4 animate-in fade-in duration-200">
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-2">One-way commute time</label>
            <div className="flex items-center gap-3">
              ...
              />
              <div className="w-20 text-center py-2 bg-white/80 rounded-lg text-slate-700 font-semibold border border-white/60">
      ...
      {!data.has_commute && (
        <div className="bg-white/50 rounded-xl p-4 text-center border border-white/60">
          <p className="text-slate-600">Nice! Working from home gives you extra flexibility 🏠</p>
        </div>
      )}
    </div>
  );
...
function CommitmentsStep({ data, updateData }: { data: ScheduleData; updateData: (updates: Partial<ScheduleData>) => void }) {
  const [showAdd, setShowAdd] = useState(false);
  const [newCommitment, setNewCommitment] = useState({ name: '', day: 'monday', start: '18:00', end: '19:00' });
  ...
  const removeCommitment = (index: number) => {
    updateData({ commitments: data.commitments.filter((_, i) => i !== index) });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-slate-800">Fixed commitments</h2>
        <p className="text-slate-500 mt-1">Any regular weekly events?</p>
      </div>

      {data.commitments.length > 0 && (
        <div className="space-y-2">
          {data.commitments.map((c, i) => (
            <div key={i} className="flex items-center gap-3 bg-white/50 rounded-xl p-3 border border-white/60">
              <div className="flex-1">
              ...
              <button onClick={() => removeCommitment(i)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
      ...
      {showAdd ? (
        <div className="bg-white/50 rounded-2xl p-4 space-y-3 animate-in fade-in duration-200 border border-white/60">
          <input
            type="text"
            ...
            className="w-full px-4 py-2.5 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700"
            autoFocus
          />
          <div className="grid grid-cols-3 gap-2">
            <select
              ...
              className="px-3 py-2.5 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700 capitalize text-sm"
            >
            ...
            <input
              ...
              className="px-3 py-2.5 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700 text-sm"
            />
            <input
              type="time"
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowAdd(false)} className="flex-1 py-2 text-slate-500 hover:bg-white/50 rounded-xl font-medium">Cancel</button>
            <button onClick={addCommitment} disabled={!newCommitment.name.trim()} className="flex-1 py-2 bg-slate-800 text-white rounded-xl font-medium hover:bg-slate-700 disabled:opacity-50">Add</button>
          </div>
        </div>
      ) : (
        <button
