    ...
    <GlassCard className="p-4">
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {post.caption && (
            <p className="mt-1.5 text-slate-600 text-sm leading-relaxed">{post.caption}</p>
          )}
          ...
          <div className="mt-3 bg-slate-100/50 rounded-xl p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-slate-700 text-sm">{post.goal_name}</span>
            </div>
            <div className="flex items-center gap-4 text-xs text-slate-500">
              {post.session_number && post.total_sessions && (
                <span>Session {post.session_number}/{post.total_sessions}</span>
              )}
              {post.duration_mins && (
                <span>{post.duration_mins}m</span>
              )}
          ...
          <div className="mt-3 flex items-center gap-4">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100/80 rounded-full text-xs font-medium text-slate-500">
              ...
              {post.streak_days > 0 && <Flame className="w-3 h-3 text-orange-500" />}
            </span>
            <button 
              onClick={() => onLike(post.id)}
              className={`flex items-center gap-1 text-sm ${post.user_has_liked ? 'text-rose-500' : 'text-slate-400'}`}
            >
...
// ============================================================
// LOGGED IN HOME (with real feed data)
// ============================================================
...
function LoggedInHome() {
  ...
  const { data: feedData, isLoading: feedLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${API_BASE}/api/posts/feed?user_id=${user.id}`);
  ...
  const { data: todayData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${API_BASE}/api/schedule/today?user_id=${user.id}`);
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {progress.streak > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 rounded-full">
                ...
                <span className="text-sm font-semibold text-slate-600">{progress.streak} days</span>
              </div>
            )}
          </div>
