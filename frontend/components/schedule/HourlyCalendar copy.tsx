...
import {
  ChevronLeft, ChevronRight, Plus, GripVertical, Check, Briefcase, Moon, Car, Users, Dumbbell, X, Clock, Target, CheckCircle, Circle, Ban, Calendar, AlertTriangle, ArrowUp, CalendarDays,
} from 'lucide-react';
...
const getBlockStyle = (type: string, category?: string, createdBy?: string): string => {
  ...
  if (type === 'event' || type === 'social') return 'bg-white/60 border-slate-300 text-slate-700 backdrop-blur-sm';
...
const getBlockBorderColor = (type: string, category?: string): string => {
  ...
  if (type === 'event' || type === 'social') return 'border-l-slate-500';
...
// ============================================================
// MOBILE AGENDA VIEW - GLASSY
// ============================================================

function MobileAgendaView({ blocks, availability, userId, onBlockClick, onAddClick, weekOffset, setWeekOffset }: {
  blocks: ScheduleBlock[]; availability?: UserAvailability | null; userId: string; onBlockClick: (block: ScheduleBlock) => void; onAddClick: () => void; weekOffset: number; setWeekOffset: (offset: number | ((prev: number) => number)) => void;
}) {
  ...
  const blocksByDate = useMemo(() => {
    ...
    Object.keys(grouped).forEach((key) => { grouped[key].sort((a, b) => new Date(a.scheduled_start).getTime() - new Date(b.scheduled_start).getTime()); });
    return grouped;
  }, [blocks]);
  ...
  const getAvailabilityForDate = useCallback((date: Date) => {
    ...
    (availability.fixed_commitments || []).forEach((commitment) => {
      if (commitment.day.toLowerCase() === dayName) { items.push({ type: 'event', start: commitment.start, end: commitment.end, label: commitment.name }); }
    });
    return items.sort((a, b) => a.start.localeCompare(b.start));
  }, [availability]);

  useEffect(() => { setTimeout(() => { todayRef.current?.scrollIntoView({ behavior: 'auto', block: 'start' }); }, 100); }, []);
  ...
  const handleDayClick = (date: Date) => {
    ...
    setTimeout(() => { const dateKey = format(date, 'yyyy-MM-dd'); document.getElementById(`agenda-date-${dateKey}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
  };

  return (
    <div className="flex flex-col h-full">
      {/* Week Selector Header - Glassy */}
                  ...
                  <div className="space-y-2">
                    {allItems.map((item, idx) => {
                      if (item.type === 'block') {
                        ...
                        return (
                          <button key={block.id} onClick={() => onBlockClick(block)} className={`w-full text-left flex gap-3 py-3 px-3 rounded-xl transition-colors backdrop-blur-sm ${isCompleted || isSkipped ? 'opacity-50' : ''} bg-white/50 hover:bg-white/70 active:bg-white/80 border border-white/60`}>
                            <div className="w-12 flex-shrink-0"><div className="text-sm text-slate-500">{startTime}</div><div className="text-xs text-slate-400">{block.duration_mins}min</div></div>
                            <div className={`flex-1 border-l-4 ${getBlockBorderColor(block.type, category)} pl-3`}>
                              <div className={`font-medium ${isCompleted || isSkipped ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                                {displayName}{block.goals?.name && displayName !== block.goals.name && <span className="text-slate-400 font-normal ml-2">• {block.goals.name}</span>}
                              </div>
                              {notesParts[1] && !isCompleted && !isSkipped && <p className="text-sm text-slate-400 mt-0.5 line-clamp-1">{notesParts[1]}</p>}
                        ...
                        return (
                          <div key={`avail-${idx}`} className="flex gap-3 py-3 px-3 opacity-60">
...
// ============================================================
// DRAGGABLE BLOCK - GLASSY
// ============================================================

function DraggableBlock({ block, top, height, onComplete, onDelete, onClick }: { block: ScheduleBlock; top: number; height: number; onComplete?: (id: string) => void; onDelete?: (id: string) => void; onClick?: (block: ScheduleBlock) => void }) {
  ...
  const style = transform ? { top: `${top}px`, height: `${Math.max(height - 2, 24)}px`, transform: `translate3d(${transform.x}px, ${transform.y}px, 0)`, zIndex: 1000 } : { top: `${top}px`, height: `${Math.max(height - 2, 24)}px`, zIndex: 10 };
  
  return (
    <div ref={setNodeRef} {...(isDraggable ? { ...attributes, ...listeners } : {})}
      className={`absolute left-1 right-1 rounded-lg border-2 px-2 py-1 overflow-hidden ${styleClass} ${isCompleted ? 'opacity-60' : ''} ${height < 40 ? 'text-xs' : 'text-sm'} ${isDragging ? 'opacity-50 shadow-2xl ring-2 ring-slate-500' : ''} ${isDraggable ? 'cursor-grab active:cursor-grabbing' : 'cursor-pointer'} hover:shadow-lg hover:z-20 transition-shadow`}
...
// ============================================================
// DROPPABLE CELL - GLASSY
// ============================================================

function DroppableCell({ id, dayIndex, hour, minute, date, isBlocked, onClick }: { id: string; dayIndex: number; hour: number; minute: number; date: Date; isBlocked: boolean; onClick: () => void }) {
...
// ============================================================
// ADD BLOCK MODAL - GLASSY
// ============================================================

function AddBlockModal({ isOpen, onClose, onAdd, selectedDay, selectedHour }: { isOpen: boolean; onClose: () => void; onAdd: (block: { type: string; days: string[]; start: string; end: string; notes?: string }) => void; selectedDay?: string; selectedHour?: number }) {
  ...
  const handleSubmit = () => { if (selectedDays.length === 0) { alert('Please select at least one day'); return; } onAdd({ type, days: selectedDays, start: startTime, end: endTime, notes: notes || undefined }); onClose(); setNotes(''); };
  const applyPreset = (preset: 'work' | 'commute-morning' | 'commute-evening') => {
    ...
    else { setType('commute'); setStartTime('18:00'); setEndTime('19:00'); selectWeekdays(); }
  };
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-end md:items-center justify-center z-50 p-0 md:p-4">
      <div className="bg-white/95 backdrop-blur-xl rounded-t-3xl md:rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border border-white/80">
        ...
        <div className="p-4 space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[{ value: 'work', label: 'Work', icon: <Briefcase className="w-4 h-4" /> }, { value: 'commute', label: 'Commute', icon: <Car className="w-4 h-4" /> }, { value: 'event', label: 'Event', icon: <Users className="w-4 h-4" /> }].map((option) => (
                <button key={option.value} onClick={() => setType(option.value as any)} className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl border-2 transition-colors ${type === option.value ? 'border-slate-500 bg-slate-100 text-slate-700' : 'border-white/60 bg-white/50 hover:border-slate-300 text-slate-600'}`}>{option.icon}{option.label}</button>
              ))}
            </div>
          </div>
          {/* Day Selection */}
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {DAYS.map((day, i) => (<button key={day} onClick={() => toggleDay(day)} className={`px-3 py-2 rounded-xl border-2 text-sm font-medium transition-colors ${selectedDays.includes(day) ? 'border-slate-500 bg-slate-100 text-slate-700' : 'border-white/60 bg-white/50 text-slate-400 hover:border-slate-300'}`}>{DAY_LABELS[i]}</button>))}
            </div>
        ...
        <div className="flex gap-3 p-4 border-t border-slate-100 sticky bottom-0 bg-white/95 backdrop-blur-xl">
          ...
          <button onClick={handleSubmit} disabled={selectedDays.length === 0} className="flex-1 px-4 py-2.5 bg-slate-700 text-white rounded-xl hover:bg-slate-600 disabled:opacity-50 font-medium">{selectedDays.length > 1 ? `Add to ${selectedDays.length} Days` : 'Add Block'}</button>
        </div>
      </div>
    </div>
  );
...
// ============================================================
// COMPLETE WITH NOTES MODAL - GLASSY
// ============================================================

function CompleteWithNotesModal({ block, onClose, onComplete, isLoading }: { block: ScheduleBlock | null; onClose: () => void; onComplete: (blockId: string, notes: string) => void; isLoading?: boolean }) {
  const [notes, setNotes] = useState('');
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-end md:items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white/95 backdrop-blur-xl rounded-t-3xl md:rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-white/80" onClick={(e) => e.stopPropagation()}>
        <div className="bg-emerald-500 p-4 text-white">
          <div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-xl"><Check className="w-5 h-5" /></div><div><h3 className="font-bold text-lg">Nice work! 🎉</h3><p className="text-sm opacity-80">{sessionName}</p></div></div>
        </div>
        <div className="p-6">
        ...
        <div className="flex gap-3 p-4 border-t border-slate-100 bg-white/50">
          ...
          <button onClick={() => onComplete(block.id, notes)} disabled={isLoading} className="flex-1 px-4 py-2.5 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 font-medium flex items-center justify-center gap-2 disabled:opacity-50"><Check className="w-4 h-4" />{isLoading ? 'Saving...' : 'Log It'}</button>
        </div>
      </div>
    </div>
  );
...
// ============================================================
// SESSION DETAIL MODAL - GLASSY
// ============================================================

function SessionDetailModal({ block, onClose, onComplete, onDelete, onPushToNextWeek, onSkip, onCompleteEarly, onReschedule }: {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-end md:items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white/95 backdrop-blur-xl rounded-t-3xl md:rounded-3xl w-full max-w-md shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-white/80" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-white/70 backdrop-blur-sm p-4 border-b border-white/40 flex-shrink-0">
          <div className="flex items-start gap-3">
            <div className="p-2 bg-white/50 rounded-xl border border-white/60">{getBlockIcon(block.type) || <Clock className="w-5 h-5 text-slate-500" />}</div>
            <div className="flex-1"><h3 className="font-bold text-lg leading-tight text-slate-700">{sessionName}</h3>{block.goals?.name && sessionName !== block.goals.name && <p className="text-sm text-slate-500 mt-1">{block.goals.name}</p>}</div>
            <button onClick={onClose} className="p-1 hover:bg-white/50 rounded"><X className="w-5 h-5 text-slate-400" /></button>
          </div>
        </div>
        {/* Action Result Banner */}
        {actionResult && <div className={`px-4 py-3 flex items-center gap-2 ${actionResult.type === 'success' ? 'bg-emerald-50/80 text-emerald-800' : actionResult.type === 'warning' ? 'bg-amber-50/80 text-amber-800' : 'bg-rose-50/80 text-rose-800'}`}>{actionResult.type === 'warning' && <AlertTriangle className="w-4 h-4" />}{actionResult.type === 'success' && <Check className="w-4 h-4" />}<span className="text-sm font-medium">{actionResult.message}</span></div>}
        {/* Details */}
        <div className="p-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-center gap-3">{isCompleted ? <><CheckCircle className="w-5 h-5 text-emerald-500" /><span className="text-emerald-600 font-medium">Completed</span></> : <><Circle className="w-5 h-5 text-slate-400" /><span className="text-slate-500">Scheduled</span></>}</div>
        </div>
        {/* Reschedule Panel */}
        {showReschedule && <div className="p-4 bg-slate-50/80 backdrop-blur-sm border-t border-slate-200/50"><div className="flex items-center gap-2 mb-3"><CalendarDays className="w-5 h-5 text-slate-500" /><p className="font-medium text-slate-700">Reschedule Session</p></div><div className="grid grid-cols-2 gap-3 mb-3"><div><label className="block text-xs text-slate-500 mb-1">Date</label><input type="date" value={rescheduleDate} onChange={(e) => setRescheduleDate(e.target.value)} className="w-full px-3 py-2 border border-white/60 bg-white/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" /></div><div><label className="block text-xs text-slate-500 mb-1">Time</label><input type="time" value={rescheduleTime} onChange={(e) => setRescheduleTime(e.target.value)} className="w-full px-3 py-2 border border-white/60 bg-white/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" /></div></div><div className="flex gap-2"><button onClick={() => setShowReschedule(false)} className="flex-1 px-3 py-2 border border-slate-200 text-slate-600 rounded-xl hover:bg-white/50 text-sm font-medium" disabled={isLoading}>Cancel</button><button onClick={handleReschedule} className="flex-1 px-3 py-2 bg-slate-700 text-white rounded-xl hover:bg-slate-600 text-sm font-medium" disabled={isLoading}>{isLoading ? 'Saving...' : 'Confirm'}</button></div></div>}
        {/* Push Confirmation */}
        {showConfirmPush && <div className="p-4 bg-amber-50/80 backdrop-blur-sm border-t border-amber-200/50"><div className="flex items-start gap-2 mb-3"><AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" /><div><p className="font-medium text-amber-800">Push to next week?</p><p className="text-sm text-amber-700 mt-1">This may delay your goal deadline.</p></div></div><div className="flex gap-2"><button onClick={() => setShowConfirmPush(false)} className="flex-1 px-3 py-2 border border-slate-200 text-slate-600 rounded-xl hover:bg-white/50 text-sm font-medium" disabled={isLoading}>Cancel</button><button onClick={handlePushToNextWeek} className="flex-1 px-3 py-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600 text-sm font-medium" disabled={isLoading}>{isLoading ? 'Pushing...' : 'Yes, Push'}</button></div></div>}
        {/* Actions */}
        ...
        {isUserBlock && !actionResult && <div className="p-4 border-t border-slate-100 bg-white/50 backdrop-blur-sm flex-shrink-0">{onDelete && <button onClick={() => { if (window.confirm('Delete this block?')) { onDelete(block.id); onClose(); }}} disabled={isLoading} className="w-full px-4 py-2 text-rose-600 hover:bg-rose-50/50 rounded-xl text-sm font-medium disabled:opacity-50">Delete Block</button>}</div>}
      </div>
    </div>
  );
}
...
// ============================================================
// APPLY TO FUTURE MODAL - GLASSY
// ============================================================

const DAY_NAMES = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

function ApplyToFutureModal({ pendingMove, onConfirm, onCancel, isLoading }: { pendingMove: { block: ScheduleBlock; newStart: string; newDayIndex: number; newHour: number; newMinute: number } | null; onConfirm: (applyToFuture: boolean) => void; onCancel: () => void; isLoading?: boolean }) {
...
export default function HourlyCalendar({ blocks, availability, userId, onBlockUpdate, weekOffset, setWeekOffset }: HourlyCalendarProps) {
  ...
  // Mutations
  const completeBlockMutation = useMutation({ mutationFn: (blockId: string) => scheduleAPI.completeBlock(blockId), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); } });
  ...
  const createBlockMutation = useMutation({ mutationFn: (block: { user_id: string; type: string; scheduled_start: string; duration_mins: number; notes?: string }) => scheduleAPI.createBlock(block), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); } });
  const createRecurringMutation = useMutation({ mutationFn: (data: { user_id: string; type: string; days: string[]; start_time: string; end_time: string; notes?: string }) => scheduleAPI.createRecurringBlock(data), onSuccess: (data) => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); alert(`✅ ${data.message}`); }, onError: (error: any) => { alert(`❌ ${error.response?.data?.message || 'Failed to create blocks'}`); } });
  const updateBlockMutation = useMutation({ mutationFn: ({ blockId, scheduled_start }: { blockId: string; scheduled_start: string }) => scheduleAPI.updateBlock(blockId, { scheduled_start }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); } });
  const updateFutureMutation = useMutation({ mutationFn: ({ blockId, scheduled_start, applyToFuture }: { blockId: string; scheduled_start: string; applyToFuture: boolean }) => scheduleAPI.updateBlockWithFuture(blockId, scheduled_start, applyToFuture), onSuccess: (data) => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); if (data.updatedCount > 1) alert(`✅ Updated ${data.updatedCount} sessions`); } });
  ...
  const pushToNextWeekMutation = useMutation({ mutationFn: async (blockId: string) => { const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/schedule/${blockId}/push-to-next-week`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' } }); if (!response.ok) throw new Error('Failed'); return response.json(); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); } });
  const skipBlockMutation = useMutation({ mutationFn: async (blockId: string) => { const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/schedule/${blockId}/skip`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' } }); if (!response.ok) throw new Error('Failed'); return response.json(); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); onBlockUpdate?.(); } });
  const completeEarlyMutation = useMutation({ mutationFn: async (blockId: string) => { const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/schedule/${blockId}/complete-early`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' } }); if (!response.ok) throw new Error('Failed'); return response.json(); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['schedule'] }); queryClient.invalidateQueries({ queryKey: ['goals'] }); onBlockUpdate?.(); } });

  const handleConfirmMove = (applyToFuture: boolean) => { if (!pendingMove) return; if (applyToFuture) updateFutureMutation.mutate({ blockId: pendingMove.block.id, scheduled_start: pendingMove.newStart, applyToFuture: true }); else updateBlockMutation.mutate({ blockId: pendingMove.block.id, scheduled_start: pendingMove.newStart }); setPendingMove(null); };
  ...
  const handleReschedule = async (blockId: string, newDateTime: string) => { await updateBlockMutation.mutateAsync({ blockId, scheduled_start: newDateTime }); };
  ...
  const availabilityBlocks = useMemo(() => {
    ...
    (availability.fixed_commitments || []).forEach((commitment) => {
      const dayIndex = DAYS.indexOf(commitment.day.toLowerCase()); if (dayIndex === -1) return;
      const start = parseTime(commitment.start); const end = parseTime(commitment.end);
  ...
  const blockedTimeSlots = useMemo(() => {
    ...
    availabilityBlocks.forEach((block) => { const startMins = block.startHour * 60 + block.startMin; for (let min = startMins; min < startMins + block.durationMins; min += SLOT_INTERVAL) blocked[`${block.dayIndex}-${Math.floor(min / 60)}-${min % 60}`] = block.type; });
    blocks.filter((b) => isBlockerType(b.type)).forEach((block) => {
      const blockDate = parseISO(block.scheduled_start);
      weekDates.forEach((date, dayIndex) => { if (isSameDay(date, blockDate)) { const startMins = blockDate.getHours() * 60 + blockDate.getMinutes(); for (let min = startMins; min < startMins + block.duration_mins; min += SLOT_INTERVAL) blocked[`${dayIndex}-${Math.floor(min / 60)}-${min % 60}`] = block.type; } });
    });
    return blocked;
  ...
  const getCollisionType = useCallback((dayIndex: number, hour: number, minute: number, durationMins: number): string | null => {
    ...
    for (let min = startMins; min < startMins + durationMins; min += SLOT_INTERVAL) { const key = `${dayIndex}-${Math.floor(min / 60)}-${min % 60}`; if (blockedTimeSlots[key]) return blockedTimeSlots[key]; }
    return null;
  }, [blockedTimeSlots]);

  const blocksByDay = useMemo(() => {
    const grouped: Record<number, ScheduleBlock[]> = {}; for (let i = 0; i < 7; i++) grouped[i] = [];
    blocks.forEach((block) => { const blockDate = parseISO(block.scheduled_start); weekDates.forEach((date, dayIndex) => { if (isSameDay(date, blockDate)) grouped[dayIndex].push(block); }); });
    return grouped;
  }, [blocks, weekDates]);

  const handleDragStart = (event: DragStartEvent) => { const block = event.active.data.current?.block as ScheduleBlock; if (block) setActiveBlock(block); };
  const handleDragOver = (event: DragOverEvent) => { /* simplified */ };
  const handleDragEnd = (event: DragEndEvent) => {
    const { active, over } = event; setActiveBlock(null);
    if (!over) return;
    ...
    if (collisionType) { setOverlapWarning({ message: `Overlaps with ${getBlockTypeName(collisionType)}. Choose a different time.`, blockingType: collisionType, show: true }); return; }
  ...
  const handleAddBlock = (blockData: { type: string; days: string[]; start: string; end: string; notes?: string }) => {
    if (blockData.days.length > 1) { createRecurringMutation.mutate({ user_id: userId, type: blockData.type, days: blockData.days, start_time: blockData.start, end_time: blockData.end, notes: blockData.notes }); }
    else {
      const dayIndex = DAYS.indexOf(blockData.days[0]); const targetDate = weekDates[dayIndex];
      const [startH, startM] = blockData.start.split(':').map(Number); const [endH, endM] = blockData.end.split(':').map(Number);
      const durationMins = (endH * 60 + endM) - (startH * 60 + startM);
      const collisionType = getCollisionType(dayIndex, startH, startM, durationMins);
      if (collisionType) { setOverlapWarning({ message: `Overlaps with ${getBlockTypeName(collisionType)}. Choose a different time.`, blockingType: collisionType, show: true }); return; }
      createBlockMutation.mutate({ user_id: userId, type: blockData.type, scheduled_start: setMinutes(setHours(targetDate, startH), startM).toISOString(), duration_mins: durationMins, notes: blockData.notes });
    }
  };

  const handleCellClick = (dayIndex: number, hour: number) => { setSelectedSlot({ day: DAYS[dayIndex], hour }); setShowAddModal(true); };
    ...
    return (
      <>
        <div className="h-[calc(100vh-140px)] relative">
          <MobileAgendaView blocks={blocks} availability={availability} userId={userId} onBlockClick={setSelectedBlock} onAddClick={() => setShowAddModal(true)} weekOffset={weekOffset} setWeekOffset={setWeekOffset} />
        </div>
    ...
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
      <div className="backdrop-blur-xl bg-white/70 rounded-2xl shadow-lg border border-white/80 overflow-hidden flex flex-col h-[calc(100vh-280px)]">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-700 text-white flex-shrink-0">
          <button onClick={() => setWeekOffset((prev) => prev - 1)} className="p-2 hover:bg-white/20 rounded-full"><ChevronLeft className="w-5 h-5" /></button>
          <div className="text-center"><h2 className="text-lg font-bold">{format(weekStart, 'MMMM yyyy')}</h2><p className="text-sm opacity-80">Week of {format(weekStart, 'MMM d')} - {format(addDays(weekStart, 6), 'MMM d')}</p></div>
          <div className="flex gap-2">{weekOffset !== 0 && <button onClick={() => setWeekOffset(0)} className="px-3 py-1 text-sm bg-white/20 hover:bg-white/30 rounded-full">Today</button>}<button onClick={() => setWeekOffset((prev) => prev + 1)} className="p-2 hover:bg-white/20 rounded-full"><ChevronRight className="w-5 h-5" /></button></div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-white/30 backdrop-blur-sm border-b border-white/40 flex-shrink-0">
          <div className="flex gap-4 text-xs text-slate-600">
            ...
            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-slate-100/80 border border-slate-300" /><span>Commute</span></div>
            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-white/70 border border-slate-400" /><span>Training</span></div>
            <div className="flex items-center gap-1 ml-2 text-slate-500"><GripVertical className="w-3 h-3" /><span>Drag to move</span></div>
          </div>
          <button onClick={() => { setSelectedSlot(null); setShowAddModal(true); }} className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 text-white rounded-full text-sm hover:bg-slate-600"><Plus className="w-4 h-4" />Add Block</button>
        </div>
        ...
        <div className="flex-1 overflow-auto flex" ref={scrollRef}>
          ...
          <div className="w-16 flex-shrink-0 border-r border-white/40 bg-white/30 backdrop-blur-sm sticky left-0 z-20">
            ...
            <div className="relative" style={{ height: `${HOURS.length * HOUR_HEIGHT}px` }}>{HOURS.map((hour, idx) => (<div key={hour} className="absolute right-2 text-xs text-slate-500 font-medium -translate-y-2" style={{ top: `${idx * HOUR_HEIGHT}px` }}>{format(setHours(new Date(), hour), 'h a')}</div>))}</div>
          </div>

          {/* Days Grid */}
          <div className="flex-1"><div className="flex min-w-[700px]">
                ...
                <div key={dayIndex} className="flex-1 min-w-[100px] border-r border-white/40 last:border-r-0">
                  ...
                  <div className="relative" style={{ height: `${HOURS.length * HOUR_HEIGHT}px` }}>
                    ...
                    {dayAvailBlocks.map((block, idx) => {
                      ...
                      return (
                        <div key={`avail-${idx}`} className={`absolute left-0 right-0 ${getBlockStyle(block.type)} border-l-4 ${getBlockBorderColor(block.type)} opacity-50 px-2 py-1 overflow-hidden pointer-events-none`} style={{ top: `${top}px`, height: `${height}px`, zIndex: 5 }}>
                          <div className="flex items-center gap-1 text-xs">{getBlockIcon(block.type)}<span className="truncate">{block.label}</span></div>
                        </div>
                      );
                    })}
                    {/* Schedule blocks */}
