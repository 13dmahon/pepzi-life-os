...
export default function ActiveSessionView({
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Calculate new progress for share modal
      const currentSession = stats?.progress.current_session_number || 1;
      const totalSessions = stats?.progress.total_sessions || 1;
      ...
      setCompletionData({
        ...
        streakDays: 1, // TODO: Calculate real streak from API
      });
  ...
  // Handle share post - converts File to base64 and calls API
  const handleSharePost = async (data: { caption: string; photoFile: File | null }) => {
    ...
    if (data.photoFile) {
      try {
        ...
        const uploadResponse = await fetch(`${API_BASE}/api/posts/upload-image`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            user_id: userId,
            image_base64: base64,
            file_name: `session_${session.id}.jpg`,
          }),
        });
    ...
    const response = await fetch(`${API_BASE}/api/posts`, {
      ...
      body: JSON.stringify({
        ...
        goal_emoji: categoryEmojis[session.category || 'skill'] || '⭐',
        session_name: session.name,
        session_number: completionData?.sessionNumber || 1,
        total_sessions: completionData?.totalSessions || 1,
        duration_mins: durationMins,
        streak_days: completionData?.streakDays || 0,
        progress_percent: completionData?.progressPercent || 0,
  ...
  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !showShareModal) onClose();
    };
    ...
    <>
      ...
      <ShareProgressModal
        ...
        sessionData={{
          ...
          goalEmoji: categoryEmojis[session.category || 'skill'] || '⭐',
          sessionName: session.name,
          sessionNumber: completionData?.sessionNumber || 1,
          totalSessions: completionData?.totalSessions || 1,
          durationMins: durationMins,
          streak: completionData?.streakDays || 0,
          progressPercent: completionData?.progressPercent || 0,
