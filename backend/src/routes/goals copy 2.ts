...
router.post('/:goalId/create-plan-with-milestones', async (req: Request, res: Response) => {
  try {
    ...
    if (simple_sessions === true && total_sessions) {
      ...
      // Generate schedule blocks from placed_sessions
      if (placed_sessions && placed_sessions.length > 0) {
        console.log(`📅 Generating ${totalSessionCount} schedule blocks from ${placed_sessions.length} placed sessions`);
        ...
        // Sort placed sessions by day order
        const sortedPlacements = [...placed_sessions].sort((a: any, b: any) => 
        ...
        while (sessionNum <= totalSessionCount) {
          for (const placement of sortedPlacements) {
            if (sessionNum > totalSessionCount) break;
            
            // Calculate the date for this placement
            const targetDay = dayMap[placement.day.toLowerCase()];
            const currentDay = today.getDay();
            
            // Days until target day this week
            let daysUntil = targetDay - currentDay;
            if (daysUntil < 0 || (daysUntil === 0 && new Date().getHours() >= placement.hour)) {
              daysUntil += 7;
            }
            ...
            sessionDate.setHours(placement.hour, placement.minute || 0, 0, 0);
            
            // Determine week number for this session
            const weekNum = Math.ceil(sessionNum / baseSessions);
            
            // Create block with simple session name
            blocks.push({
              goal_id: goalId,
              user_id: goal.user_id,
              type: 'goal_session',
              ...
              status: 'scheduled',
              // Store session info in notes field (name|||description|||tip format)
          ...
          if (weekOffset > 200) {
            console.error('❌ Schedule generation exceeded 200 weeks - stopping');
            break;
          }
        }
        
        // Delete any existing blocks for this goal and insert new ones
        if (blocks.length > 0) {
          ...
          for (let i = 0; i < blocks.length; i += batchSize) {
            ...
            if (blockError) {
              console.error(`❌ Failed to insert block batch ${i / batchSize + 1}:`, blockError);
            }
          }
          
          console.log(`✅ Created ${blocks.length} schedule blocks for simple sessions`);
        }
      }
